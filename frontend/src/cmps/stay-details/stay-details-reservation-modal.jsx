...
export function StayDetailsOrderModal({ stay }) {
  ...
  function calculateDays() {
    const days =
      (new Date(filterBy.checkOut) - new Date(filterBy.checkIn)) /
      (1000 * 60 * 60 * 24);
    // setDaysCount(days)
        ...
        <section className="order-form">
          <div className="order-info mar-b24">
            ...
            <div className="fs14 fw600">
              <MdStar />
              4.88 ·
              <button onClick={openReviewModal} className="sml-review-btn">
            ...
            <div className="order-details-setting mar-b16">
              <button className="order-form-btn up" onClick={toggleDatePicker}>
                <div className="date-container flex column">
                  ...
                  <div className="upp-left-14-600 mar-r8">
                    {filterBy?.checkIn ? (
                      new Date(filterBy.checkIn).toLocaleDateString()
                    ) : (
                      <span className="grey-71">Add date</span>
                    )}
                  </div>
                </div>
                <div className="date-container flex column">
                  <div className="upp-left-14-600 bold fs9">CHECKOUT</div>
                  <div className="upp-left-14-600">
                    {filterBy?.checkOut ? (
              ...
              <button
                className="order-form-btn down flex space-between center"
                onClick={() => setIsGuestsClicked(!isGuestsClicked)}
              >
                <div className="date-container flex column guests">
                  ...
                  <div className="upp-left-14-600 fs14 ">
                    {filterBy?.adults > 1 || filterBy?.children > 0
                      ? filterBy.adults + filterBy.children + " guests"
                      : "1 guest"}
                  </div>
                </div>
                {isGuestsClicked ? (
                  <BiChevronUp className="guests-arrow" />
                ) : (
                  <BiChevronDown className="guests-arrow" />
                )}
              </button>
              {isGuestsClicked && (
                <div className="guests-counter-container details-guest">
                  <GuestsCounter
                    filter={filterBy}
          ...
          <div className="order-form-pricing mar-b24">
            ...
            <div>
              ...
              {filterBy.checkOut
                ? utilService.toActualPrice(
                    (stay.price *
                      (new Date(filterBy.checkOut) -
                        new Date(filterBy.checkIn))) /
                      (1000 * 60 * 60 * 24)
                  )
                : 0}
            </div>
